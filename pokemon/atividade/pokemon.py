...
def buscar_conteudo_da_url(valor: None, api='pokemon'):
    if type(valor) == str:
        valor = valor.lower()
    url = f"http://pokeapi.co/api/v2/{api}/{valor}/"
...
def nome_do_pokemon(numero):
    dados = buscar_conteudo_da_url(numero)
    return dados['name']
...
def numero_do_pokemon(nome):
    dados = buscar_conteudo_da_url(nome)
    if dados['id']:
        return dados['id']
    else:
        return dados
...
def color_of_pokemon(nome):
    dados = buscar_conteudo_da_url(nome, "pokemon-species")
    if dados['color']['name']:
...
def tipos_do_pokemon(nome):
    ...
    for key in dados['types']:
        name = key['type']['name']
        lista.append(dic_tipos[name])
    return lista
...
def evolucao_anterior(nome):
    dados = buscar_conteudo_da_url(nome, "pokemon-species")
    if not dados['evolves_from_species']:
        return None
    return dados['evolves_from_species']['name']
...
def evolucoes_proximas(nome):
    nome = nome.lower()
    dado = requests.get(f'https://pokeapi.co/api/v2/pokemon-species/{nome}/')
    if dado.status_code == 404:
        raise PokemonNaoExisteException
    else:
        dado = dado.json()
        url = dado['evolution_chain']['url']
        dado = requests.get(url)
        dado = dado.json()['chain']
        lista = []
        if dado['species']['name'] != nome:
            for indice in dado['evolves_to']:
...
É possível determinar o nível (1 a 100) em que um pokémon se encontra 
com base na quantidade de pontos de experiência que ele tem.
...
def nivel_do_pokemon(nome, experiencia):
    dados = buscar_conteudo_da_url(nome, 'pokemon-species')
    url = dados['growth_rate']['url']
    dados = requests.get(url)
    dados = dados.json()

    if dados['levels'][0]['experience'] == experiencia:
        return dados['levels'][0]['level']

    if dados['levels'][-1]['experience'] <= experiencia:
        return dados['levels'][-1]['level']

    for dici in dados['levels']:
        experience, level = dici.values()
        if experience > experiencia:
            return level-1
